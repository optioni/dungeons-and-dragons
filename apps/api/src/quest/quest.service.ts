...
export class QuestService {
        ...
        try {
            const quest = await this.em.transactional(async (em) => {
                ...
                for (const link of pendingLinks) {
                    const qe = em.create(QuestEntity, { quest, questId, entityType: link.entityType, entityId: link.entityId });
                    em.persist(qe);
                }
    ...
    async updateQuestObjective(objectiveId: number, status: QuestObjectiveStatus): Promise<QuestObjectiveResult | QuestError> {
        const objective = await this.objectiveRepo.findOne(objectiveId);
        if (!objective) return { success: false, reason: 'OBJECTIVE_NOT_FOUND' };
    ...
    async runAutoChecker(campaignId: number): Promise<AutoCheckerResult> {
        ...
        if (activeQuests.length === 0) return { questCompleted: null };

        const questIds = activeQuests.map((q) => (q as unknown as { id: number }).id);
        const incompleteObjectives = await this.objectiveRepo.find({
            questId: { $in: questIds },
            status: QuestObjectiveStatus.INCOMPLETE,
        });

        if (incompleteObjectives.length === 0) return { questCompleted: null };
        ...
        for (const quest of activeQuests) {
            const questId = (quest as unknown as { id: number }).id;
            if (!affectedQuestIds.has(questId)) continue;
    ...
    private async evaluateObjective(
        ...
        switch (objective.type) {
            case QuestObjectiveType.REACH_LOCATION: {
                if (!campaign || objective.entityId === null) return false;
                return campaign.currentLocationId === objective.entityId;
            }
            case QuestObjectiveType.NPC_DEAD: {
                if (objective.entityId === null) return false;
            ...
            case QuestObjectiveType.HAVE_ITEM: {
                if (!character || objective.entityId === null) return false;

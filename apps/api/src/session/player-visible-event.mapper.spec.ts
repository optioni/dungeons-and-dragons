...
describe('PlayerVisibleEventMapper', () => {
    ...
    it('maps inventory, resource, quest, travel, and discovery events', () => {
        ...
        ).toMatchObject({ category: 'INVENTORY', kind: 'ITEM_GAINED' });
        ...
        ).toMatchObject({ category: 'RESOURCE', kind: 'SPELL_SLOT_USED' });
        ...
        ).toMatchObject({ category: 'QUEST', kind: 'QUEST_CREATED', title: 'Quest started: Find the courier' });
        ...
        ).toMatchObject({ category: 'TRAVEL', kind: 'TRAVEL_COMPLETED' });
        ...
        expect(
            ...
            title: 'Location discovered: Blackfen Crossing',
        });
    });

...
const DM_TOOLS: Anthropic.Tool[] = [
        ...
        input_schema: {
            ...
            properties: {
                scene_type: {
                    ...
                    description: 'The new scene type',
                },
            },
            required: ['scene_type'],
        },
    },
    {
        ...
        description:
            'Presents action options to the player at a decision point. Include pending_check if an action will trigger a skill/ability check, telegraphing the DC so the player knows the stakes before committing. Use when multiple valid paths exist (talk to NPC, sneak past guards, fight, negotiate).',
        input_schema: {
            type: 'object' as const,
            properties: {
                actions: {
                    ...
                    description: 'Concise action labels (3–5 options)',
                },
                pending_check: {
                    type: 'object' as const,
                    ...
                    properties: {
                        skill: {
                            ...
                            description: 'Skill name (e.g. "Persuasion") — mutually exclusive with ability',
                        },
                        ability: {
                            type: 'string' as const,
                            description: 'Ability name (STR/DEX/CON/INT/WIS/CHA) — mutually exclusive with skill',
                        },
    ...
    // ── Dice ─────────────────────────────────────────────────────────────────
    {
        name: 'roll_dice',
        description:
            'Rolls a dice expression for NPC actions, random events, or other mechanics outside character skill checks. Examples: "2d6" for wandering monster check, "1d20+5" for an NPC attack roll, "3d6" for random encounter damage. Use check_skill/check_ability for character-initiated rolls with DCs.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    // ── Combat ───────────────────────────────────────────────────────────────
    {
        name: 'start_combat',
        description:
            'Initiates combat: rolls initiative for all participants and sets combat state. Call this the moment combat begins (enemy appears, player draws weapon, ambush triggers).',
        input_schema: {
            type: 'object' as const,
            properties: {
                participants: {
    ...
    },
    {
        name: 'apply_damage',
        description:
            'Applies damage to a combatant after a hit lands. Always include damage_type (slashing, piercing, bludgeoning, fire, cold, acid, poison, radiant, necrotic, psychic, thunder, force, etc.).',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'heal',
        description:
            'Restores HP from a healing spell, potion, or ability. Capped at max HP. Use during combat for healing spells/potions; use outside combat to represent natural rest recovery via take_short_rest or take_long_rest.',
        input_schema: {
            type: 'object' as const,
            properties: {
                target_id: { type: 'string', description: '"character" or the NPC encounter key' },
    ...
    },
    {
        ...
        description:
            'Applies a D&D 5e condition (Poisoned, Stunned, Restrained, Blinded, Charmed, etc.) from spells, abilities, or environmental effects. Conditions modify future rolls and actions.',
        input_schema: {
            type: 'object' as const,
            properties: {
                target_id: { type: 'string', description: '"character" or the NPC encounter key' },
                condition: {
                    type: 'string',
                    description:
                        'Condition name (Poisoned, Stunned, Restrained, Blinded, Charmed, Frightened, Prone, etc.)',
                },
            },
    ...
    },
    {
        ...
        description:
            'Removes a condition from a combatant when a spell ends it, an action removes it, or a save succeeds.',
        input_schema: {
            type: 'object' as const,
            properties: {
                target_id: { type: 'string', description: '"character" or the NPC encounter key' },
    ...
    },
    {
        ...
        description:
            'Rolls a death saving throw (d20 + 0) when the character is at 0 HP and making death saves. Success: +1 success counter; Failure: +1 failure counter. 3 successes = stabilized; 3 failures = dead.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'stabilise',
        description:
            "Stabilizes a downed character (at 0 HP) by stopping death saves and setting HP to 1. Use when a healing spell reaches them, an ally uses Healer's Kit, or another stabilization ability triggers.",
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'instant_death',
        description:
            'Kills a character instantly from massive environmental damage (falling from a cliff, lava, crushed by a collapsing structure). Use when damage in a single hit exceeds max HP by 10+.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'end_combat',
        description:
            'Ends combat when all enemies are defeated, fled, surrendered, or the encounter is resolved. Clears combat state and resets initiative.',
        input_schema: { type: 'object' as const, properties: {} },
    },
    // ── Rest ─────────────────────────────────────────────────────────────────
    {
        ...
        description:
            'Takes a 1-hour short rest: player spends hit dice to recover HP, short-rest ability recharges trigger. No day advance; no world tick. Use when player wants to rest briefly during the same day (after combat, mid-adventure).',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    // ── Travel ───────────────────────────────────────────────────────────────
    {
        name: 'travel_to',
        description:
            'Moves the player to a previously discovered location (one already in the campaign database). May trigger a random encounter en route. Call when player chooses to travel or when moving between discovered locations.',
        input_schema: {
            type: 'object' as const,
            properties: {
                location_id: {
                    type: 'number',
                    description: 'ID of the target location (must be previously discovered or created)',
                },
            },
    ...
    },
    {
        ...
        input_schema: {
            ...
            properties: {
                ...
                source: {
                    type: 'string',
                    description: 'Discovery source: EXPLORATION, NPC, MAP, QUEST, PLAYER_ACTION, WORLD_TICK, SETUP',
                },
    ...
    },
    {
        ...
        input_schema: {
            ...
            properties: {
                ...
                current_state: {
                    type: 'string',
                    description: 'Current narrative state (SAFE, TENSE, RUINED, etc.) (optional)',
                },
                connected_location_ids: {
                    type: 'array',
                    items: { type: 'number' as const },
                    description: 'IDs of adjacent locations (optional)',
                },
                parent_location_id: {
                    type: 'number',
                    description: 'Parent settlement ID for sub-locations (optional)',
                },
            },
    ...
    // ── Items ─────────────────────────────────────────────────────────────────
    {
        ...
        input_schema: {
            ...
            properties: {
                ...
                item_type: {
                    type: 'string',
                    description: 'Item type (WEAPON, ARMOR, POTION, MISC, MAGIC, QUEST, DOCUMENT, CONSUMABLE, etc.)',
                },
    ...
    },
    {
        name: 'give_item',
        description:
            'Transfers an item from the campaign registry into a character or NPC inventory. Always call after create_item. Specify either to_character_id or to_npc_id (not both).',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'equip_item',
        description:
            'Equips a character item to a specific slot (MAIN_HAND, OFF_HAND, ARMOR, ACCESSORY). Use when character equips a weapon, dons armor, or puts on a ring.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'unequip_item',
        description:
            'Unequips a character item from its equipped slot. Use when character removes armor, drops a weapon, or takes off an accessory.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'buy_item',
        description:
            'Character purchases an item from an NPC merchant—deducts gold from character, item goes to inventory. Use in SETTLEMENT scenes during shopping. Handles currency automatically; only specify if merchant price differs from item value.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'sell_item',
        description:
            'Character sells an item to an NPC merchant—adds gold to character, item leaves inventory. Use in SETTLEMENT scenes during shopping. Merchant determines buyback price.',
        input_schema: {
            type: 'object' as const,
            properties: {
                npc_id: { type: 'number', description: 'Merchant NPC ID' },
    ...
    },
    {
        ...
        description:
            "Updates an NPC merchant's inventory (stock and prices). Replaces existing stock. Use during world tick to refresh merchant wares or in SETTLEMENT scenes to establish a shop.",
        input_schema: {
            type: 'object' as const,
            properties: {
                npc_id: { type: 'number', description: 'Merchant NPC ID' },
    ...
    },
    {
        name: 'place_item',
        description:
            'Places an item at an overworld location (on ground, on a shelf, left behind by an NPC). Stacks quantity if item is already there. Use when NPCs leave items or DM places environmental treasure.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'take_item',
        description:
            'Transfers an item from an overworld location into character inventory. Use when player picks up a placed item. Opposite of place_item.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    // ── Leveling ──────────────────────────────────────────────────────────────
    {
        ...
        description:
            'Signals character has earned a level: pauses session for player to choose ability score improvements (ASI) or a feat, and roll for HP. Call when character XP reaches threshold.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'apply_level_up',
        description:
            'Applies level-up choices after player selects ASI (ability score improvements) or a feat, and rolls HP on hit die. Call to finalize the level-up.',
        input_schema: {
            type: 'object' as const,
            properties: {
                character_id: { type: 'number', description: 'Character leveling up' },
                ability_score_improvements: {
                    type: 'object' as const,
                    description: 'Map of ability name to +1/+2 increase',
                },
    ...
    },
    {
        name: 'use_spell_slot',
        description:
            'Expends a spell slot when character casts a spell. Always call when a spell is cast to track remaining slots. Use after describing the spell effect.',
        input_schema: {
            type: 'object' as const,
            properties: {
                ...
                level: {
                    type: 'number',
                    description: "Spell slot level (1–9, or 0 for cantrips which don't consume slots)",
                },
            },
            required: ['level'],
        },
    },
    {
        name: 'prepare_spells',
        description:
            'Sets prepared spells for Wizard/Cleric/Druid characters after long rest. Player chooses spells they can cast. Use after long_rest if character is a preparation-based caster.',
        input_schema: {
            type: 'object' as const,
            properties: {
                character_id: { type: 'number', description: 'Spellcaster character' },
    ...
    },
    {
        ...
        description:
            'Signals character must choose prepared spells after long rest (for Wizard/Cleric/Druid). Pauses session until player selects spells.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    // ── World / NPCs ──────────────────────────────────────────────────────────
    {
        ...
        input_schema: {
            ...
            properties: {
                ...
                disposition: {
                    type: 'string',
                    description: 'Attitude toward the player (e.g. "friendly", "hostile", "wary") (optional)',
                },
                personality_traits: {
                    type: 'array',
                    items: { type: 'string' as const },
                    description: 'Adjectives or phrases describing behaviour (optional)',
                },
    ...
    },
    {
        name: 'update_npc',
        description:
            "Updates an NPC's disposition, location, agenda, or alive status after significant changes (disposition shift from story, NPC dies/recovers, agenda changes, NPC moves locations). Use sparingly—only for material story changes.",
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'add_to_party',
        description:
            "Makes an NPC a companion who travels with the player, fights in combat, and accompanies them. Suspends NPC's world tick agenda. Use when NPC joins the party (romance, quest requirement, voluntary companionship).",
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        ...
        description:
            "Removes an NPC companion from the party (leaves voluntarily, dies, betrays player, or quest completes). Resumes NPC's world tick agenda.",
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        ...
        description:
            'Updates a location\'s narrative state to reflect changes. When to use: when a location\'s condition materially changes (fire spreads, building collapses, area becomes safe, creatures move in). Examples: "BURNING", "RUINED", "OCCUPIED", "SAFE", "HOSTILE", "CURSED". This ensures future visits to the location remember what changed.',
        input_schema: {
            type: 'object' as const,
            properties: {
                ...
                state: {
                    type: 'string',
                    description: 'New narrative state (e.g. TENSE, SAFE, RUINED, BURNING, OCCUPIED)',
                },
            },
    ...
    },
    {
        ...
        input_schema: {
            ...
            properties: {
                ...
                disposition: {
                    type: 'string',
                    description: 'New disposition (e.g. "allied", "friendly", "neutral", "hostile", "hunted")',
                },
            },
    ...
    },
    {
        ...
        description:
            'Creates an active world event: a crisis, opportunity, or background development with stakes and deadline. Use when significant world changes happen (bandits blockade a road, plague spreads, faction power shift, natural disaster, antagonist makes a move). Provides context for future adventures.',
        input_schema: {
            type: 'object' as const,
            properties: {
                ...
                deadline_in_game_date: {
                    type: 'string',
                    description: 'In-game date deadline for resolution (optional)',
                },
                source: {
                    type: 'string',
                    description: 'Event source: PLAYER_ACTION, WORLD_TICK, ANTAGONIST, CATASTROPHE',
                },
            },
    ...
    },
    {
        ...
        description:
            'Marks an active world event as resolved and records the outcome. Use when event deadline is met, crisis is averted, opportunity seized, or event no longer relevant.',
        input_schema: {
            type: 'object' as const,
            properties: {
                ...
                outcome: {
                    type: 'string',
                    description: 'Narrative outcome (what happened, why it happened, consequences)',
                },
            },
    ...
    },
    {
        ...
        description:
            "Marks the current antagonist plan stage as complete and advances to the next stage. Use when player completes a major quest related to the antagonist or antagonist's plan progresses. Moves the campaign toward climax.",
        input_schema: { type: 'object' as const, properties: {} },
    },
    {
        name: 'record_lore',
        description:
            "Appends a discovered fact to the campaign's permanent lore document. Use for world-building facts the player discovers (history, secrets, prophecies, discovered truths). Persists for future reference and recap.",
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    // ── Dungeon ───────────────────────────────────────────────────────────────
    {
        name: 'enter_dungeon',
        description:
            'Enters a dungeon and places player at entry room. Initializes dungeon exploration session. Call when player decides to explore a dungeon.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'move_to_room',
        description:
            'Moves player to an adjacent dungeon room. Call each time player moves (not just describing movement, but actually changing rooms). Updates exploration state.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'exit_dungeon',
        description:
            'Exits the current dungeon and returns player to the overworld. Use when player leaves dungeon intentionally or is forced out. Call before set_scene_type to EXPLORATION/SETTLEMENT.',
        input_schema: { type: 'object' as const, properties: {} },
    },
    {
        name: 'spawn_encounter',
        description:
            'Triggers a combat encounter in a dungeon room. Use when a room contains a keyed encounter or wandering monster emerges. Call spawn_encounter, then set_scene_type to COMBAT.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        ...
        description:
            "Updates a dungeon room's state when it materially changes (EXPLORED, CLEARED, FLOODED, COLLAPSED, etc.). Remembers state for future visits.",
        input_schema: {
            type: 'object' as const,
            properties: {
                ...
                state: {
                    type: 'string',
                    description: 'New room state (EXPLORED, CLEARED, FLOODED, COLLAPSED, LOCKED, TRAPPED, etc.)',
                },
            },
    ...
    },
    {
        name: 'add_room_item',
        description:
            'Places an item in a dungeon room. Use when DM adds treasure, plants loot, or introduces an item the player can find. Only valid during active dungeon session.',
        input_schema: {
            type: 'object' as const,
            properties: {
                ...
                container_name: {
                    type: 'string',
                    description: 'Container description (e.g. "chest", "corpse", "altar") (optional)',
                },
            },
    ...
    },
    {
        name: 'loot_room',
        description:
            'Transfers an item from a dungeon room to character inventory when player picks it up. Use loot_room (not create_item + give_item) for dungeon loot. Only valid during active dungeon session.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    // ── Memory ────────────────────────────────────────────────────────────────
    {
        name: 'record_memory',
        description:
            'Persists a fact about an entity (character detail, location lore, faction belief, item history). Use for world-building facts the DM wants to remember long-term. Searchable via search_memories.',
        input_schema: {
            type: 'object' as const,
            properties: {
                subject_type: {
                    type: 'string',
                    enum: ['character', 'npc', 'location', 'faction', 'item', 'general'],
    ...
    },
    {
        ...
        description:
            'Performs a semantic search over recorded memory facts to retrieve relevant lore, history, or relationships. Use when the DM needs to recall something specific (e.g., "what do we know about the Thieves Guild?").',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    // ── Quests ────────────────────────────────────────────────────────────────
    {
        ...
        input_schema: {
            ...
            properties: {
                ...
                npcs: {
                    type: 'array',
                    description: 'NPCs to create alongside the quest',
                    items: { type: 'object' as const },
                },
                locations: {
                    type: 'array',
                    description: 'Locations to create alongside the quest',
                    items: { type: 'object' as const },
                },
                items: {
                    type: 'array',
                    description: 'Items to create alongside the quest',
                    items: { type: 'object' as const },
                },
                world_events: {
                    type: 'array',
                    description: 'World events to create alongside the quest',
                    items: { type: 'object' as const },
                },
            },
    ...
    },
    {
        name: 'complete_quest',
        description:
            'Marks a quest as completed, awards XP and gold rewards to character. Call when player accomplishes all quest objectives or quest giver confirms completion.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        name: 'fail_quest',
        description:
            'Marks a quest as failed (objective becomes impossible, deadline passed, quest giver dies, player betrays quest giver). Use only when quest can no longer be completed.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    },
    {
        ...
        description:
            'Updates a quest objective status as player makes progress (PENDING → COMPLETED when objective is achieved, or PENDING → FAILED if objective becomes impossible). Auto-completes quests when all objectives are done.',
        input_schema: {
            type: 'object' as const,
            properties: {
    ...
    // ── Campaign ──────────────────────────────────────────────────────────────
    {
        name: 'end_campaign',
        description:
            'Permanently ends the campaign: character dies (PERMADEATH mode), story reaches definitive conclusion, or player requests end. Archives the campaign and shows epitaph to player.',
        input_schema: {
            type: 'object' as const,
            properties: {
                ...
                reason: {
                    type: 'string',
                    description: 'Reason for ending (e.g. "character death", "quest complete", "player choice")',
                },
    ...
    },
    {
        ...
        description:
            'Adjusts campaign-level settings (e.g., toggle random travel encounters). Use to customize difficulty or pacing.',
        input_schema: {
            type: 'object' as const,
            properties: {
...
export class DmOrchestrator {
    ...
    async runTurn(sessionId: number, playerInput: string): Promise<void> {
        ...
        this.logger.log(
            `Context loaded: sessionId=${sessionId} sceneType=${sceneType} historyMessages=${historyMessages.length} npcMemories=${npcMemories ? 'yes' : 'no'}`,
        );
        ...
        const messages: Anthropic.MessageParam[] = [...priorHistory, { role: 'user', content: playerInput }];
        ...
        try {
            ...
            this.logger.log(
                `DM turn complete: sessionId=${sessionId} loopIterations=${loopIterations.count} duration=${duration}ms`,
            );
    ...
    private async runToolLoop(
        ...
        this.logger.log(
            `Anthropic stream start: sessionId=${sessionId} model=${this.dmModel} iteration=${loopIterations.count}`,
        );
        ...
        try {
            ...
            this.logger.log(
                `Anthropic stream complete: sessionId=${sessionId} provider=anthropic model=${this.dmModel} stopReason=${finalMessage.stop_reason} duration=${streamDuration}ms`,
            );
    ...
    private async handleToolUseLoop(
        ...
        for (const block of finalMessage.content) {
            ...
            if (block.name === 'suggest_actions') {
                ...
                if (suggestActionsResult.success) {
                    ...
                    this.logger.warn(
                        `Tool dispatch failed: sessionId=${sessionId} tool=${block.name} errorCode=${(suggestActionsResult as { errorCode?: string }).errorCode ?? 'unknown'} message=${(suggestActionsResult as { message?: string }).message ?? ''}`,
                    );
                }
            ...
            try {
                ...
                if (toolSuccess === false) {
                    ...
                    this.logger.warn(
                        `Tool dispatch failed: sessionId=${sessionId} tool=${block.name} errorCode=${errorCode} errorMessage=${errorMessage} input=${JSON.stringify(block.input)}`,
                    );
                } else {
                    this.logger.log(`Tool dispatch: sessionId=${sessionId} tool=${block.name} success=true`);
                }
                ...
                const diceRollContent = this.extractDiceRollContent(
                    block.name,
                    block.input as Record<string, unknown>,
                    result,
                );
        ...
        await this.runToolLoop(
            sessionId,
            system,
            [...messages, { role: 'assistant', content: finalMessage.content }, { role: 'user', content: toolResults }],
            narrativeRef,
            loopIterations,
        );
    }
    ...
    private async loadSceneNpcMemories(campaignId: number, playerInput: string): Promise<string | undefined> {
        ...
        for (const npc of npcs) {
            ...
            const memories = await this.npcMemoryService.searchNpcMemories(npc.id, playerInput, limit - lines.length);
    ...
    private getToolSuccessStatus(toolName: string, result: { success?: boolean }): string | undefined {

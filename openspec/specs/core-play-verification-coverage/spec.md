# Spec: Core Play Verification Coverage

## Purpose

Ensure the core play loop and surrounding navigation pages have deterministic automated coverage, so regressions in campaign navigation, player-facing state pages, and the play route itself are caught without requiring live LLM calls in routine test runs.

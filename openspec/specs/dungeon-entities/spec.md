# Dungeon Entities Spec

## Purpose

Defines the persistent data model for dungeon-mode gameplay — the `Dungeon` aggregate, dungeon rooms (as `Location` rows scoped to a dungeon), keyed `RoomEncounter` records, and pre-placed `RoomItem` treasure. Covers schema, relationships, and GraphQL query surface.

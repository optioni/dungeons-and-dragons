...
Defines the campaign-scoped world entities that represent the physical and political landscape of a campaign: locations, maps, factions, world events, and location discovery records. This spec covers persistence requirements, the discovery model (derived exclusively from `LocationDiscovery` records), and the GraphQL API surface for querying world data.

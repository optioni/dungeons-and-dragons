...
Provides read-only access to D&D 5e System Reference Document (SRD) data — classes, races, spells, monsters, equipment, and conditions — seeded from `dnd5eapi.co` on first migration and exposed via GraphQL queries for use by the character creation system and game engine.

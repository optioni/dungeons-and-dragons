...
Defines how campaigns are created, owned, queried, and tracked through the setup wizard. A campaign is the top-level container for all play state — world entities, NPCs, sessions, and memory all belong to a campaign. This spec covers persistence, GraphQL API surface, and web routing for the setup flow.

...
The web application SHALL render a `CombatPanel` component that is visible only when `GameSession.sceneType = COMBAT`. The panel SHALL slide in from the left of the play layout when combat starts (i.e. when a stream event signals the scene type transition to COMBAT) and slide out when the scene type returns to a non-combat value. The panel SHALL NOT block or replace the central narrative column.
...
#### Scenario: Combat panel slides in when scene type transitions to COMBAT
...
The `CombatPanel` SHALL render quick action buttons — Attack, Cast Spell, Dash, Dodge, and Other — below the initiative list. Clicking a quick action SHALL populate the player text input with a suggested action prompt (e.g. "I attack with my longsword") so the player can review and submit it. The buttons SHALL be disabled while a DM turn is in progress.

#### Scenario: Quick action button pre-fills the player input
- **WHEN** the player clicks the Attack quick action button
- **THEN** the text input is populated with an attack action prompt and focus moves to the input

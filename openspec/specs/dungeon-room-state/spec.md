# Dungeon Room State Spec

## Purpose

Defines the lifecycle of room states within a dungeon — from initial `UNEXPLORED` through `EXPLORED`, `CLEARED`, `LOCKED`, and `TRAPPED`. Covers the `update_room_state` DM tool, automatic transitions on first entry, the rule that `CLEARED` is always explicit (never derived), and the GraphQL query surface for room state filtering.

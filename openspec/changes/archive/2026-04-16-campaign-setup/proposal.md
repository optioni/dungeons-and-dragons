...
- `WorldEvent` entity
- Campaign creation flow: create campaign shell → character creation → tone selection → LLM generates story concepts → player picks one → LLM generates world seed → all entities persisted
- Dashboard page (`/`) and unified campaign setup wizard (`/campaign/[id]/setup`)
